...
def get_random_images(count):
    image_cycle = itertools.cycle(restaurant_images)  # Cycles through images when needed
    return [next(image_cycle) for _ in range(count)]
...
def get_real_time_restaurants(location, food_type, limit=5):
    ...
    if response.status_code == 200:
        ...
        for i, venue in enumerate(data.get("results", [])):
            ...
            # Append restaurant details WITHOUT rating
            restaurants.append({"name": name, "address": address, "image": image_url})
    ...
    for i, idx in enumerate(top_indices):
        ...
        if (restaurant["Restaurant"] in filtered_df["Restaurant"].values and
            ...
            recommendations.append({
                ...
                "image": random_images[i] if i < len(random_images) else "static/images/default.jpg"
            })
